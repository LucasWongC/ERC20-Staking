...
class StakingDetail extends Component {
  ...
  fetchStakeInfo() {
    ...
    this.state.contractInstance.methods
      .getStakeInfo(this.state.stakeID)
      .call()
      .then((result) => {
        //1 - Amount Staked
        //2 - isWithdrawed
  ...
  getPeriods() {
    this.state.contractInstance.methods
      .calculatePeriods(this.state.stakeID)
      .call()
      .then((result) => {
  ...
  getAlreadyWithdrawed() {
    this.state.contractInstance.methods
      .getAlreadyWithdrawedAmount(this.state.stakeID)
      .call()
      .then((result) => {
        console.log(result);
        if (Boolean(result)) {
  ...
  render() {
    ...
    return (
      ...
      >
        <Grid item>
          <Typography>
            <Trans i18nKey="stake_detail.amountStaked" /> :{" "}
            <b>{this.state.viewAmount}</b>
          </Typography>
          <Typography>
            <Trans i18nKey="stake_detail.periodElasped" />:{" "}
            <b>{this.state.periods}</b>
          </Typography>
          <Typography>
            <Trans i18nKey="stake_detail.penalty" />:{" "}
            <b>{this.state.penalty.toString()}</b>
          </Typography>

          {/*<Typography><Trans i18nKey="stake_detail.gain" />: <b color="green">{this.state.gain}%</b></Typography>*/}
          <Typography>
            <Trans i18nKey="stake_detail.withdrawed" />:{" "}
            <b>{this.state.wasWithdraw ? "YES" : "NO"}</b>
          </Typography>
        </Grid>

        <Grid item>
          <Typography color="textSecondary">
            <Trans i18nKey="stake_detail.creationTime" />:{" "}
            <b>{this.state.creationTimeString}</b>
          </Typography>
        </Grid>
        <Grid item>
