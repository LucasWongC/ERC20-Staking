...
class ApproveTranfer extends React.Component {
  ...
  componentDidMount() {
    ...
    this.state.tokenInstance.Transfer().on("data", (event) => {
      ...
      if (sender === this.state.accounts[0]) this.hasAllowance();
    });
  ...
  async hasAllowance() {
    if (this.state.tokenInstance === null) return;

    await this.state.tokenInstance
      .allowance(this.state.accounts[0], this.state.stakingAddress)
      .then((response) => {
        ...
        if (!amountAllowed.isZero() && !amountAllowed.isNeg()) {
          ...
          console.log(
            "Has the allowance to manage " + result.toString() + " tokens"
          );
          this.setState({
            hasAllowance: true,
          ...
          this.props.enqueueSnackbar(
            "Has the allowance to manage " + result.toString(10) + " tokens",
            {
              variant: "success",
              anchorOrigin: {
                vertical: "bottom",
  ...
  async getApprove() {
    ...
    await this.state.tokenInstance
      .approve(
        ...
        this.state.approvedAmount.toString(),
      ...
      allowanceMessage = (
        <div>
          <Typography variant="body1" component="h3">
            <Trans i18nKey="approvementPanel.approvementStatus" />{" "}
            <b>{this.state.viewApprovedAmount}</b> {this.state.tokenSymbol}{" "}
            tokens
          </Typography>
        </div>
      );
      approveButtonText = (
        <Trans i18nKey="approvementPanel.editInternalButton" />
      );
    } else {
      allowanceMessage = (
        <Typography variant="body1" component="h3">
          Does not have the approvement
        </Typography>
      );
        ...
        >
          <Grid item>
            <Typography variant="h5" component="h5">
              <Trans i18nKey="approvementPanel.title" />
            </Typography>
          </Grid>
          <Grid item>
            {allowanceMessage}
            {!this.state.editAmountMode && this.state.hasAllowance && (
              <Button color="secondary" onClick={this.handleEditButton}>
                <Trans i18nKey="approvementPanel.editExternalButton" />
              </Button>
            )}
          </Grid>
            ...
            <div>
              ...
              >
                ...
                {this.state.editAmountMode && (
                  <Button color="secondary" onClick={this.handleEditButton}>
                    <Trans i18nKey="approvementPanel.closeButton" />
                  </Button>
                )}
              </Grid>
            </div>
