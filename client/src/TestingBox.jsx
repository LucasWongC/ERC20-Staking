...
class TestingBox extends Component {
  ...
  async getApprove() {
    ...
    await this.state.tokenInstance
      .approve(this.state.stakingAddress, this.state.finalAmount.toString(), {
    ...
    this.state.contractInstance.methods
      ...
      .then((result) => {
        ...
        this.props.enqueueSnackbar(
          "Correctly staked " + this.state.amountToStake + " tokens",
          {
            variant: "success",
            anchorOrigin: {
              vertical: "bottom",
  ...
  async batchTransaction() {
    ...
    try {
      ...
      var tokenContract = new this.web3.eth.Contract(
        BitcoinCompanyNetworkTestToken.abi,
        this.state.tokenAddress
      );
      tokenContract.options.from = this.state.account;

      var stakingContract = new this.web3.eth.Contract(
        StakingContract.abi,
      ...
      batch.add(
        stakingContract.methods
          .stakeToken(
            this.state.finalAmount.toString(),
      ...
      return (
        <div>
          ...
          {this.state.approvementConfirmed && (
            <Typography>
              Approvement granted, waiting for the contract transaction...
            </Typography>
          )}
        </div>
      );
