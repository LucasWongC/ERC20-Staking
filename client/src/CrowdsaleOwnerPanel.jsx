...
class CrowdsaleOwnerPanel extends React.Component {
  ...
  handleSetRate(event) {
    ...
    this.state.web3.eth.getAccounts().then((account) => {
      ...
      this.state.crowdsaleInstance
        .setRate(this.state.web3.utils.toWei(this.state.newRate, "wei"), {
          from: account[0],
        })
        .then((res) => {
          console.log(res);
  ...
  changeOwner() {
    this.state.web3.eth.getAccounts().then((account) => {
      //this.state.crowdsaleInstance.setRate(this.state.web3.utils.toBN(this.state.newRate).toString(), {from: account[0]}).then( (res) => {
      this.state.crowdsaleInstance
        .transferOwnership(this.state.newOwner, { from: account[0] })
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Grid item>
            <Typography>
              Current exchange rate: <b>{this.state.currentRate}</b> wei
            </Typography>
          </Grid>
          <Grid item>
            <Typography>
              Current exchange liquidity: <b>{this.state.availableBalance}</b>
            </Typography>
          </Grid>
          <Grid item>
