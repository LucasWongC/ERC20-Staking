...
class ReferralBox extends Component {
  ...
  fetchTotalReward() {
    ...
    this.state.contractInstance.methods
      .getLowestStake(this.state.referrals[0])
      .call()
      .then((rawResult) => {
  ...
  fetchStakeReward() {
    this.state.contractInstance.methods
      .calculateRewardReferral(this.state.referrals[0])
      .call()
      .then((rawResult) => {
        console.log(rawResult);
      })
          ...
          >
            <Grid item>
              <Typography variant="h4" component="h4">
                <Trans i18nKey="referral_panel.referralTitle" />
              </Typography>
            </Grid>
            <Grid item>
              <Typography>
                <Trans i18nKey="referral_panel.myReferralLabel" />:{" "}
              </Typography>
            </Grid>
            <Grid item>
            ...
            <Grid item>
              <CopyToClipboard
                ...
                onCopy={() => {
                  this.props.enqueueSnackbar(
                    <Trans i18nKey="referral_panel.copyMessage" />,
                    {
                      variant: "info",
                      anchorOrigin: {
                        vertical: "bottom",
              ...
              >
                <Button variant="outlined">
                  <Trans i18nKey="referral_panel.copyButton" />
                </Button>
              </CopyToClipboard>
            </Grid>
            {this.state.referrals !== null ? (
              <div>
                <Grid item>
                  {!this.state.loading ? (
                    (this.referral_list = this.state.referrals.map(
                      (ref_address) => (
                        <ListItem button>
                          <ReferralEntry
                ...
                <Grid item>
                  <Typography>
                    <Trans i18nKey="referral_panel.totalAvailavelReward" />:{" "}
                    <b>{this.state.viewReferralReward}</b>
                  </Typography>
                </Grid>
                <Grid item>
                  <Button
              ...
              <Grid item>
                ...
                <Typography color="error">
                  <Trans i18nKey="referral_panel.noReferral" />
                </Typography>
              </Grid>
            )}
          </Grid>
