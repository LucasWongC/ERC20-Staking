...
class StakingForm extends Component {
  ...
  handleReferralCheckbox(event) {
    ...
    else this.setState({ showReferralInput: false });
  }
  ...
  handleSubmit(event) {
    ...
    if (typeof this.state.finalAmount == "undefined") {
      ...
      this.props.enqueueSnackbar(
        <Trans i18nKey="staking_form.errorUndefinedStake" />,
        {
          variant: "warning",
          anchorOrigin: {
            vertical: "bottom",
    ...
    this.checkAllowance().then(() => {
      ...
      if (this.state.amountToStake > this.state.allowanceValue) {
        this.props.enqueueSnackbar(
          <Trans i18nKey="staking_form.errorNoAllowance" />,
          {
            variant: "error",
            anchorOrigin: {
              vertical: "bottom",
      ...
      if (this.state.amountToStake > this.state.tokenBalance) {
        this.props.enqueueSnackbar(
          <Trans i18nKey="staking_form.errorInsufficientFunds" />,
          {
            variant: "error",
            anchorOrigin: {
              vertical: "bottom",
  ...
  async checkAllowance() {
    ...
    await this.state.tokenInstance
      .allowance(this.state.account, this.state.stakingAddress)
      .then((amountAllowed) => {
        if (!amountAllowed.isZero()) {
          ...
          let viewBalance = BigNumber(amountAllowed.toString()).divide(
            decimals
          );
  ...
  async activateStake() {
    ...
    this.state.contractInstance.methods
      .stakeToken(this.state.finalAmount.toString(), _referralAddress)
      ...
      .then((result) => {
        ...
        this.props.enqueueSnackbar(
          <Trans i18nKey="staking_form.correctStake" />,
          {
            variant: "success",
            anchorOrigin: {
              vertical: "bottom",
      ...
      .catch((err) => {
        ...
        this.setState({ loading: false });
        this.props.enqueueSnackbar(
          <Trans i18nKey="staking_form.stakingPending" />,
          {
            variant: "warning",
            anchorOrigin: {
              vertical: "bottom",
              ...
              >
                <Grid item>
                  <Typography variant="body1" component="p">
                    <Trans i18nKey="staking_form.referralCheckbox" />
                  </Typography>
                </Grid>
                <Grid item>
                  <Checkbox
