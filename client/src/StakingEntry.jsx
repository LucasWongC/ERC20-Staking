...
class StakingEntry extends Component {
  ...
  async getStakeAmount() {
    this.state.contractInstance.methods
      .getCurrentStakeAmount(this.state.stakeID)
      .call()
      .then((result) => {
      ...
      .catch((err) => {
        console.log(
          "Unable to get stake amound for ID " + this.state.stakeID + "; " + err
        );
      });
  }
  ...
  async getCompoundReward() {
    this.state.contractInstance.methods
      .calculateCompoundInterest(this.state.stakeID)
      .call()
      .then((result) => {
        console.log(result);
      })
  ...
  handleDetailView() {
    if (this.state.moreDetail === true) this.setState({ moreDetail: false });
    else this.setState({ moreDetail: true });
  }
    ...
    return (
      ...
      >
        <ExpansionPanelSummary
          ...
          id="panel-header"
        >
          {this.state.isArchived ? <NotInterestedIcon /> : <AttachMoneyIcon />}
          <Typography variant="body1" component="h6" color="textPrimary">
            {" "}
            Amount Staked: <b>{this.state.amountStaked}</b>
          </Typography>
        </ExpansionPanelSummary>
        <ExpansionPanelDetails>
          <Grid container>{this.detailBox}</Grid>
        </ExpansionPanelDetails>
      </ExpansionPanel>
    );
  }
