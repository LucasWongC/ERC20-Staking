...
class CrowdsalePanel extends React.Component {
  ...
  async getCurrentRate() {
    this.state.crowdsaleInstance.getCurrentRate
      .call()
      .then((res) => {
  ...
  handleBuy(event) {
    ...
    this.state.web3.eth.getAccounts().then((account) => {
      ...
      console.log(
        this.state.web3.utils.toWei(this.state.amountOfWEI, "wei").toString()
      );
      this.state.crowdsaleInstance
        .buyTokens(account[0], {
          value: this.state.web3.utils.toWei(this.state.amountOfWEI, "wei"),
          from: account[0],
        })
        .then((res) => {
          this.props.enqueueSnackbar(
            <Trans i18nKey="crowdsale.snackbarSuccess" />,
            {
              variant: "success",
              anchorOrigin: {
                vertical: "bottom",
        ...
        .catch((err) => {
          this.props.enqueueSnackbar(
            <Trans i18nKey="crowdsale.snackbarFailure" />,
            {
              variant: "error",
              anchorOrigin: {
                vertical: "bottom",
  ...
  handleChange(event) {
    try {
      ...
      if (integerAmount > this.state.availableBalance) {
        ...
        this.props.enqueueSnackbar(
          <Trans i18nKey="crowdsale.liquidityLimitReached" />,
          {
            variant: "info",
            anchorOrigin: {
              vertical: "bottom",
      ...
      <Grow in={true}>
        ...
        >
          ...
          >
            ...
            <Grid item>
              <Typography>
                <Trans i18nKey="crowdsale.liquidity" />:{" "}
                {this.state.availableBalance}
              </Typography>
            </Grid>
            <Grid item>
              <Typography>
                <Trans i18nKey="crowdsale.currentRate" />:{" "}
                <b>{this.state.currentRate}</b>
              </Typography>
            </Grid>
            <Grid item>
              <TextField
