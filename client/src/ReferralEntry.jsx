...
class ReferralEntry extends Component {
  ...
  fetchReferralReward() {
    this.state.contractInstance.methods
      .calculateRewardReferral(this.state.referredAddress)
      .call()
      .then((rawResult) => {
  ...
  render() {
    return (
      <Container maxWidth="sm">
        <Link
          href={"https://etherscan.io/address/" + this.state.referredAddress}
          id="list-elem"
        >
          <ListItemText primary={this.state.referredAddress} />
        </Link>
        <Typography color="textSecondary">
          <Trans i18nKey="referral_panel.availableReward" />:{" "}
          {this.state.availableReward}
        </Typography>
      </Container>
    );
  }
