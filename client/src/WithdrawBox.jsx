...
class WithdrawBox extends Component {
  ...
  withdrawTokens() {
    ...
    this.state.contractInstance.methods
      .returnTokens(this.state.stakeID)
      .send({ from: this.state.selectedAccount })
      .then((result) => {
        ...
        if (this.isWithdrawed(this.state.stakeID)) {
          ...
          this.props.enqueueSnackbar(
            <Trans i18nKey="withdrawBox.tokenSuccesWithdrawMsg" />,
            {
              variant: "success",
              anchorOrigin: {
                vertical: "bottom",
          ...
          console.log(
            "Unable to withdraw tokens, probably the time is not expired yet"
          );
          this.props.enqueueSnackbar(
            <Trans i18nKey="withdrawBox.tokenSuccesWithdrawMsg" />,
            {
      ...
      >
        <Grid item>
          <Typography color="primary">
            <Trans i18nKey="withdrawBox.withdrawLabel" />
          </Typography>
        </Grid>
        <Grid item>
          <Button
          ...
          >
            <Trans i18nKey="stake_detail.withdraw" />
          </Button>
        </Grid>
      </Grid>
    );
